...
if (isset($_POST['registeruser'])) {
  ...
  if (empty($email)) { array_push($errors, "Emel pengguna diperlukan"); }
  ...
  if ($user) { // if user exists
    if ($user['username'] === $username) {
      array_push($errors, "Nama pengguna telah wujud. Sila guna nama pengguna yang lain");
    }
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO user (username, fullname, position, phonenum, email, password) 
...
if (isset($_POST['simpan'])) {
		...
		mysqli_query($db, "UPDATE user SET username='$username', fullname='$fullname', position='$position', phonenum='$phonenum', email='$email' WHERE userID=$userID");
		$_SESSION['message'] = "Profil telah dikemas kini"; 
...
if (isset($_POST['login_user'])) {
  ...
  if (empty($password)) {
  	array_push($errors, "Kata laluan dperlukan");
  }
